...
<div class="container-lg">
    <div class="row justify-content-center">
      <div class="col-12 col-lg-10 col-xl-8">
        ...
        <!-- Form -->
        <form class="tab-content py-6" id="wizardSteps" action="{{ route('psicologos.update', $publicador->id) }}" method="POST">
          @csrf
          @method('PUT')
          ...
          <div class="tab-pane fade show active" id="wizardStepOne" role="tabpanel" aria-labelledby="wizardTabOne">
           ...
           <div class="col-12 col-md-6" style="margin-left: 25%;">
            ...
            <!-- Input -->
            <input type="text" value="{{ $publicador->numero_colegiatura }}" name="numero_colegiatura" class="form-control" placeholder="########">
            ...
            <div class="form-group">
              ...
              <textarea name="enfermedades_tratadas" id="enfermedades_tratadas" cols="80" rows="5" >{{$publicador->enfermedades_tratadas}}</textarea>
            ...
            <div class="row">
                <div class="col-12 col-md-6">
                  ...
                  <div class="form-group">
                    ...
                    <!-- Input -->
                    <input type="text" name="facebook" value="{{ $publicador->facebook }}" class="form-control">
                ...
                <div class="col-12 col-md-6">
                  ...
                  <div class="form-group">
                    ...
                    <!-- Input -->
                    <input type="text" value="{{$publicador->instagram}}" name="instagram" class="form-control">
          ...
          <div class="tab-pane fade" id="wizardStepTwo" role="tabpanel" aria-labelledby="wizardTabTwo">
            ...
            <div class="row justify-content-center">
              ...
              <div class="col-12 col-md-6" style="margin-left: 25%;">
                ...
                <!-- Input -->
                <input type="text"  name="formacion" value="{{$publicador->formacion}}" class="form-control" placeholder="Instituto Universitario">
              ...
              <div class="form-group">
                ...
                <small class="form-text text-body-secondary">
                 ...
                 <!-- Quill -->
                 <!-- Quill -->
                 <textarea name="experiencia" id="experiencia" cols="80" rows="5">{{$publicador->experiencia}}</textarea>
            ...
            <div class="table-responsive">
              ...
              <table class="table table-bordered">
                ...
                <tbody>
                  @foreach($dias as $dia)
                  @php
                    // Buscamos si para este día existe ya un horario
                    $horario = (isset($publicador) && $publicador->horarios)
                               ? $publicador->horarios->firstWhere('dia', $dia)
                               : null;
                  @endphp
                  <tr>
                    <td>{{ $dia }}</td>
                    <td>
                      <label class="custom-toggle">
                        <input type="checkbox" 
                               name="horarios[{{ $dia }}][active]" 
                               value="1" 
                               id="dia_{{ $dia }}"
                               @if($horario) checked @endif>
                        <span class="custom-toggle-slider rounded-circle"></span>
                      </label>
                      <!-- Campo oculto para enviar el nombre del día -->
                      <input type="hidden" name="horarios[{{ $dia }}][dia]" value="{{ $dia }}">
                    </td>
                    <td>
                      <select name="horarios[{{ $dia }}][hora]" class="form-control">
                        <option value="">Selecciona</option>
                        @for ($i = 9; $i <= 11; $i++)
                        <option value="{{ sprintf('%02d:00', $i) }}" @if($horario && $horario->hora == sprintf('%02d:00', $i)) selected @endif>{{ $i }}:00 AM</option>
                        <option value="{{ sprintf('%02d:30', $i) }}" @if($horario && $horario->hora == sprintf('%02d:30', $i)) selected @endif>{{ $i }}:30 AM</option>
                        @endfor
                      </select>
                    </td>
                    <td>
                      <select name="horarios[{{ $dia }}][hora_salida]" class="form-control">
                        <option value="">Selecciona</option>
                        @for ($i = 1; $i <= 11; $i++)
                        <option value="{{ sprintf('%02d:00', $i) }}" @if($horario && $horario->hora == sprintf('%02d:00', $i)) selected @endif>{{ $i }}:00 AM</option>
                        <option value="{{ sprintf('%02d:30', $i) }}" @if($horario && $horario->hora == sprintf('%02d:30', $i)) selected @endif>{{ $i }}:30 AM</option>
